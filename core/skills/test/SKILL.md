...
Before any BRAINSTORM phase: skip if `--skip-brainstorm` was passed OR `.claude/workflows.yml` has `workflows.test.require_brainstorm: false`. Mark as `SKIPPED` in Phase History.
