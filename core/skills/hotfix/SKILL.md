...
3. Create `.workflows/current-state.md` with: workflow name, feature name, first phase (DIAGNOSE) as ACTIVE, started/updated timestamps, empty Phase History table, empty Completed Steps, Artifacts, and Context sections.
...
Before any BRAINSTORM phase: skip if `--skip-brainstorm` was passed OR `.claude/workflows.yml` has `workflows.hotfix.require_brainstorm: false`. Mark as `SKIPPED` in Phase History.
