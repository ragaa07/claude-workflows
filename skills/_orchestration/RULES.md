...
Read `.workflows/config.yml` (if exists) -> `workflows.<skill>`. If no config file, use defaults from `${CLAUDE_PLUGIN_ROOT}/config/defaults.yml`:
...
1. Determine project type from `.workflows/config.yml` -> `project.type`. If no config file, use `${user_config.project_type}`.
2. Read language rules from `${CLAUDE_PLUGIN_ROOT}/rules/` based on project type:

   | project_type | Files to read from `${CLAUDE_PLUGIN_ROOT}/rules/` |
   |---|---|
...
3. If team is set (from config `project.team` or `${user_config.team}`), also read: `${CLAUDE_PLUGIN_ROOT}/teams/<team>/rules/team-conventions.md`
4. Follow every DO/DON'T while implementing.

**Before creating a PR** (every workflow that ends with PR):

1. Always read: `${CLAUDE_PLUGIN_ROOT}/reviews/general-checklist.md`
2. Read language-specific checklists from `${CLAUDE_PLUGIN_ROOT}/reviews/` based on project type:

   | project_type | Checklists to read from `${CLAUDE_PLUGIN_ROOT}/reviews/` |
   |---|---|
...
3. If team is set, also read: `${CLAUDE_PLUGIN_ROOT}/teams/<team>/reviews/team-review-checklist.md`
...
| Config file missing | Guide user: `/claude-workflows:setup` to initialize project |
...
After completing a workflow (Rule 5), check `.workflows/config.yml` → `chains`:
