...
Apply plan step by step. Read `${CLAUDE_PLUGIN_ROOT}/rules/` for language-specific patterns before starting.
...
**Quality gate**: Load `${CLAUDE_PLUGIN_ROOT}/reviews/general-checklist.md` and the language-specific checklist from `${CLAUDE_PLUGIN_ROOT}/reviews/`. Verify all High/Critical items pass. Confirm full test suite and build are green.
