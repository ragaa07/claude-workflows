...
**What You Get**: 20 workflow skills (as plugin slash commands), 5 brainstorming techniques, orchestration rules enforced on every execution, review checklists (general + language-specific), coding rules per language, multi-session persistence, and a single YAML config file.
...
1. `.workflows/config.yml` (project-level overrides)
2. `${user_config.*}` (plugin userConfig values set during install)
3. Plugin defaults (`${CLAUDE_PLUGIN_ROOT}/config/defaults.yml`)
...
**Orchestration rules** (`${CLAUDE_PLUGIN_ROOT}/skills/_orchestration/RULES.md`) apply to every execution: phase output documents written to `.workflows/<feature>/`, state updated at every transition, build/test commands auto-detected, quality gate enforced, workflow chaining supported.
...
### Coding Rules (`${CLAUDE_PLUGIN_ROOT}/rules/`)
...
### Review Checklists (`${CLAUDE_PLUGIN_ROOT}/reviews/`)
...
| `/claude-workflows:hotfix [--crash <id>]` | Emergency production fix |
...
| `/claude-workflows:template` | Generate documents from templates |
...
```
/claude-workflows:extend-feature <name> [--describe "what to add"]
```
...
```
/claude-workflows:hotfix [--crash <id>] [--error "description"] [--log <path>]
```
...
```
/claude-workflows:new-project [project-path]
```
...
|-------------------------------|---------|
| `skills/*/SKILL.md` | 20 workflow skills (auto-discovered) |
| `skills/_orchestration/RULES.md` | Global orchestration rules |
| `templates/*.tmpl` | 5 document templates |
